...
-- Start the matching hardcoded server for the current filetype when the binary exists.
local function start_for_buf(bufnr, capabilities)
...
-- Keep the old explicit helper commands because they are useful when debugging PATH issues.
local function setup_commands(start_with_capabilities)
  vim.api.nvim_create_user_command('LspStart', function()
    start_with_capabilities(vim.api.nvim_get_current_buf())
...
function M.setup()
  ...
  vim.api.nvim_create_autocmd('FileType', {
    ...
    callback = function(args)
      start_for_buf(args.buf, capabilities)
    end,
  })

  setup_commands(function(bufnr)
    start_for_buf(bufnr, capabilities)
  end)
end
